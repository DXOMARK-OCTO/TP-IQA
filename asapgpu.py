'''
Code source (edited): https://github.com/gfxdisp/asap 
'''
import torch
...
def kl_div_torch(mean_1, std_1, mean_2, std_2):
...
def ASAP(cmp_matrix: np.ndarray, mst_mode=True, cuda=False):
    ...
    if np.sum(cmp_matrix) == 0 and mst_mode:
        rp = np.random.permutation(M)
        rord = np.random.permutation(M-1)
        pairs_to_compare = [(rp[rord[k]], rp[rord[k]+1]) for k in range(M-1)]
        return np.array(pairs_to_compare)
    ...
    # Compute expected information gain for all possible outcomes
    kl_divs = prob_cmps(normal0)
    

    for cc in range(0,len(I)):
        total = kl_div_torch(normal0.loc, normal0.scale, 
                                             normal.loc[cc], normal.scale[cc])
        if not torch.isnan(total).any():
            kl_divs[I[cc],J[cc]] *= total
        else:
            print("Impossible combination: ", cc)
